...
def verify_pairs(min_zscore, half_life_max):
     ...
     for i in range(len(major_pairs_y)):
          for j in range(len(major_pairs_x)):
                    ...
                    print(f"Dependente {major_pairs_y[i]} e independente {major_pairs_x[j]}")
                    print(f"Zscore {z_score} e  min zscore {min_z_score}")
...
def volume_adjust(symbolY,symbolX,hedge_ratio):
  ...
  df_Y = get_data(symbolY, mt5.TIMEFRAME_D1, periods, 0).dropna()
  df_X = get_data(symbolX, mt5.TIMEFRAME_D1, periods, 0).dropna()

  min_lot_Y = mt5.symbol_info(symbolY).volume_min
  min_lot_X = mt5.symbol_info(symbolX).volume_min

  # Example price series for the two assets
  prices_asset_y = np.array(df_Y['close'])
  prices_asset_x = np.array(df_X['close'])

  # Calculate returns
  returns_asset_y = np.diff(prices_asset_y) / prices_asset_y[:-1]
  returns_asset_x = np.diff(prices_asset_x) / prices_asset_x[:-1]

  # Scale the returns
  scaler = StandardScaler()
  scaled_returns_asset_y = scaler.fit_transform(returns_asset_y.reshape(-1, 1)).flatten()
  scaled_returns_asset_x = scaler.fit_transform(returns_asset_x.reshape(-1, 1)).flatten()

  # Fit GARCH(1,1) model to each asset's scaled returns
  garch_model_asset_y = arch_model(scaled_returns_asset_y, vol='Garch', p=1, q=1)
  garch_model_asset_x = arch_model(scaled_returns_asset_x, vol='Garch', p=1, q=1)

  garch_fit_asset_y = garch_model_asset_y.fit(disp="off")
  garch_fit_asset_x = garch_model_asset_x.fit(disp="off")

  # Extract the conditional volatilities (standard deviations)
  volatility_asset_y = garch_fit_asset_y.conditional_volatility[-1]
  volatility_asset_x = garch_fit_asset_x.conditional_volatility[-1]
  
  total_investment = mt5.account_info().equity
  # Adjust lot sizes based on volatility
  volatility_ratio = volatility_asset_y / volatility_asset_x
  investment_asset_y = total_investment / (1 + hedge_ratio * volatility_ratio * prices_asset_x / prices_asset_y)
  investment_asset_x = total_investment - investment_asset_y

  lot_size_asset_y = abs(investment_asset_y / prices_asset_y)
  lot_size_asset_x = abs(investment_asset_x / prices_asset_x)
  
  volume_y = total_investment*(lot_size_asset_y)/450
  volume_x = total_investment*(lot_size_asset_x)/450
  
  volume_y = abs(float(math.ceil(volume_y[-1:][0])))
  volume_x = abs(float(math.ceil(volume_x[-1:][0])))
  
  volume_y = 6.0
  volume_x = 5.0
  
  print(f"Volume for {volume_y} and {volume_x} volatility ratio {volatility_ratio} asset y volume {lot_size_asset_y[-1:]} asset x volume {lot_size_asset_x[-1:]}")
...
def trailing_stop(symbol,position_type,price_current,stop_loss,ticket):
    ...
    print(f"Trailing para {symbol} com profit {profit} new stop {new_stop_loss} old stop {stop_loss} ")
...
trading_time_end = trading_time_start + timedelta(hours=3,minutes=50)
      ...
      if (total_positions > 0):
          ...
          for position in positions:
              if position.comment == 'independent':
                  ...
                  print(f"Aplicando trailing para {open_position_x} {open_position_x}")
          ...
          if not (open_position_x is None or open_position_y is None):        
            ...
            print(f"Pearson coefficient {slope}")
